...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++) {
		...
		model->materials[i].shininess = 1.0;
    }
...
/* glmUnitizeOnlyMove: "unitize" a model by translating it to the origin and
 * scaling it to fit in a unit cube around the origin.   Returns the
 * scalefactor used.
 *
 * model - properly initialized GLMmodel structure 
 */
GLfloat
glmUnitizeOnlyMove(GLMmodel* model)
{
    GLuint  i;
    GLfloat maxx, minx, maxy, miny, maxz, minz;
    GLfloat cx, cy, cz;
...
GLvoid
glmMove(GLMmodel* model, GLfloat deltaX, GLfloat deltaY, GLfloat deltaZ)
{
	GLuint i;

	for (i = 1; i <= model->numvertices; i++) {
		model->vertices[3 * i + 0] += deltaX;
		model->vertices[3 * i + 1] += deltaY;
		model->vertices[3 * i + 2] += deltaZ;
	}
}
